...
app.get('/:verb/:adjective/:noun', (req, res) => { const { verb, adjective, noun } = req.params;
    const responseMessage = `Congratulations on starting a new project called ${verb}-${adjective}-${noun}!`;
    res.send(responseMessage);
});

app.get('/', (req, res) => { res.send(` <h1>Welcome to the Bug Tracker</h1>
...
app.get('/bugs', (req, res) => {
    //the initial 99 bugs text
    res.send(`  <h1>99 little bugs in the code</h1>
        <a href="/bugs/101">Pull one down, patch it around</a>
    `);
});

app.get('/bugs/:numberOfBugs', (req, res) => {  const numberOfBugs = parseInt(req.params.numberOfBugs, 10);
    if (numberOfBugs >= 200) {
    ...
    
    {
        ...
        //each time the number of bugs is added by 2 until it reaches 200.  Unless its counting down
        res.send(`
